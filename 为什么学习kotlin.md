...
而动态语言：如（JavaScript, Groovy）则是动态强类型语言，即我们在声明一个属性的时候，不用指定其类型，在程序运行时，对属性的初始化时会确定其类型，对类型合法性的检测是在运行时进行的。动态强类型可以让我们写出更佳简短的代码，但是，因为缺少静态检测，如果没有非常完备的代码覆盖测试，很容易由于拼写错误或者对使用的其它人编写的的程序不够熟悉，而造成问题或者闪退，而且往往发生在程序被release之后。
