...
这样设计的目的在于，对状态部分，我们保证对其访问的线程安全性，并实现对状态改变的监听，而控制模块则负责根据不同的状态来调动不同的功能函数来相应和处理，并将功能函数的结果更新到状态中去，这样的设计，我们可以很容易的编写测试代码来分别测试控制和功能部分的代码。每一个功能的函数的只能非常单一且做到自包含，而控制函数则更加容易进行测试，我们可以方便的模拟状态事件，并调用的时候注入功能函数来验证控制逻辑。而响应式函数编程为我们提供了完整的控制功能分离的解决方案。
