...
- 高阶函数（Higher-Order functions）高阶函数的定义是，一个函数以其它函数作为入参，或者以函数作为结果return。高阶函数是通过First Class Function机制实现的即，函数首先是一个类，我们可以把函数理解成只有一个成员方法的类。函数式编程思想中，经常会通过高阶函数的方式抽象数据处理的机制，例如，集合类中的map， filter等方法实现了低层次的对数据的处理流程，但是，这些函数依赖开发人员实现的高阶函数来提供对特定集合中的数据的处理逻辑，并可以通过高阶函数来调整整个低层次处理流程的运转机制。
 
- Lambdas 函数式语言提供 Higher-Order functions 特性，Lambdas 可以理解为 Higher-Order function 的极简形式，即将一个代码块作为一个函数的入参。Java8中开始引入了Lambdas，从此使得Java语言有了函数式编程的特性，之前我们只能通过匿名内部类来实现类似的目的。Lambdas使得我们的代码更加简洁，可读性更高。
注意：Lambdas表达式实现的逻辑不宜过重，一般推荐使用lambdas表达式作为胶水将事件和处理逻辑进行绑定，即表达式只实现对功能函数的调用。或者实现对特定数据的解析和转化，例如实现集合类的filter函数的对数据的过滤规则。
