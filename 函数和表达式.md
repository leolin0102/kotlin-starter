...
filter函数的作用是筛选指定的数据，map的作用是对统一对集合中的数据进行转换。reduce的作用则相当于对数据进行聚合，我们可以把reduce同样理解为对集合进行转换，与map不同的是，map是针对数据个体进行转换，而reduce是针对整个集合的数据进行归纳，统计和汇总的一个过程。
...
接下来我们分别实现上面的两个高阶函数，为了突出lambdas的特性我们这里忽略掉实现细节。
