...
如果上面的例子操作的集合类的数据非常大则我们可以这样优化：

<pre><code>
val result = lists.asSequence().filter { it.age > 30 }.map { it.name }.toList()
</code></pre>

我们先将Iterator转换成一个Sequence，并通过lambda调用链来告诉Sequence处理每一个数据的全部步骤，当我们调用toList的时候，每一个对象都会被lambda链处理并得到最终的结果。
...
从定义中可以发现Sequence实例本身持有着Iterator迭代器，因此，Sequence实际上是一个对迭代器的封装，与直接在Iterator上调用filter或者map不同的是后者会在函数调用的时候运行lambda函数对集合进行迭代并创建新的List集合存放处理后的结果并返回，而前者则不会，Sequence则是将lambda保存起来延后执行，因此对Sequense链式调用实际上是构造了一个lambda调用链，当我们最终调用toList函数的时候，Sequence才会遍历数据，并将数据一个一个的传给lambda调用链来处理数据并将结果放入新的List中。

你可以对任何Collection子类调用asSequence函数得到一个Sequence，最终通过调用toList函数得到结果。


### forEach函数
=======
函数式风格中，经常会出现链式调用的风格如list.filter { it. age > 35 }.map { it.name }，之所以可以链式调用的原因是
>>>>>>> afc283129c58ac2d7b1033d83d945f97b64e9f30
