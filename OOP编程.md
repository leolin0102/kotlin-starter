...
定义ProcessInstance接口的作用是隔离模型和驱动模型流转引擎的实现，使得我们可以随时根据需求和选择来切换流程引擎的具体实现，这样使得我们同样的模型可以被应用到不同环境下的流程引擎中去，比如手机端，专有服务器或者云端。这样只要我们能保证基于ProcessInstance接口来实现对流程的管理和控制，就可以随时切换成不同的环境和引擎而达到不对现有业务进行大范围调整的风险。
